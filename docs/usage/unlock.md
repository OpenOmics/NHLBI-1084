...
This part of the documentation describes options and concepts for <code>iso-seek <b>unlock</b></code> sub command in more detail. With minimal configuration, the **`unlock`** sub command enables you to unlock a pipeline output directory. 
...
Unlocking iso-seek pipeline output directory is fast and easy! In its most basic form, <code>iso-seek <b>unlock</b></code> only has *one required input*.

## 2. Synopsis
```text
$ ./iso-seek unlock [-h] --output OUTPUT
```
...
iso-seek unlock --output /data/$USER/output
```